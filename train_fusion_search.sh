...
mkdir -p "$dir/checkpoints_multimodal_again"
...
nvidia-smi --query-gpu=timestamp,name,utilization.gpu,utilization.memory,memory.used,memory.total,temperature.gpu \
         --format=csv -l 30 > "$dir/slurm_logs_multimodal/gpu_usage_${FUSION_TYPE}.log" &
GPU_MONITOR_PID=$!

# Start resource monitoring
vmstat -n 30 > "$dir/slurm_logs_multimodal/resource_usage_${FUSION_TYPE}.log" &
